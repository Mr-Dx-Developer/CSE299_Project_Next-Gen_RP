...
RegisterNetEvent('hospital:client:RevivePlayer', function()
    ...
    if hasItem then
        ...
        if player ~= -1 and distance < 5.0 then
            ...
            QBCore.Functions.Progressbar("hospital_revive", Lang:t('progress.revive'), 12000, false, true, {
    ...
    CreateThread(function()
        ...
        while check do
            if IsControlJustPressed(0, 38) then
                ...
                elseif variable == "ground" then
                    TriggerEvent('qb-ambulancejob:elevator_main2')
                elseif variable == "main2" then
                    TriggerEvent('qb-ambulancejob:elevator_ground')
                end
            end
            Wait(1)
        end
...
RegisterNetEvent('qb-ambulancejob:elevator_ground', function()
    local ped = PlayerPedId()
    for k, _ in pairs(Config.Locations["ground"]) do
        DoScreenFadeOut(500)
        while not IsScreenFadedOut() do
            Wait(10)
        end
...
RegisterNetEvent('qb-ambulancejob:elevator_main2', function()
    ...
    for k, _ in pairs(Config.Locations["main2"]) do
        ...
        local coords = Config.Locations["ground"][currentHospital]
        SetEntityCoords(ped, coords.x, coords.y, coords.z, 0, 0, 0, false)
        SetEntityHeading(ped, coords.w)
...
if Config.UseTarget then
    CreateThread(function()
        ...
        for k, v in pairs(Config.Locations["ground"]) do
            exports['qb-target']:AddBoxZone("ground" .. k, vector3(v.x, v.y, v.z), 2, 2, {
                name = "ground" .. k,
                debugPoly = false,
                heading = -20,
                minZ = v.z - 2,
                ...
                options = {
                    {
                        type = "client",
                        event = "qb-ambulancejob:elevator_ground",
                        icon = "fas fa-hand-point-up",
                        label = "Take Elevator",
                        job = "ambulance"
                    },
        ...
        for k, v in pairs(Config.Locations["main2"]) do
            exports['qb-target']:AddBoxZone("main2" .. k, vector3(v.x, v.y, v.z), 1.5, 1.5, {
                name = "main2" .. k,
                debugPoly = false,
                heading = -20,
                minZ = v.z - 2,
