	...
	if Patient then
		if Player.PlayerData.job.name == "ambulance" or QBCore.Functions.HasItem(src, "firstaid", 1) then
			if oldMan then
				...
				TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['firstaid'], "remove")
				TriggerClientEvent('hospital:client:Revive', Patient.PlayerData.source)
				Patient.Functions.RemoveMoney('bank', 1000, 'Treatment Money')
				Player.Functions.AddMoney('bank', 1000, 'Treatment Money')
				TriggerClientEvent('QBCore:Notify', src, 'You got treatment money $1000', 'success')
				TriggerClientEvent('QBCore:Notify', playerId, 'Removed Money $1000', 'error')
			end
		else
